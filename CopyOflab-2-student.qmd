...
library("here")
library("readr")
#data_csv <-read_csv(here::here("Wk2_R","lab-2-clee387","data","surveys.csv"))
...
There are 30,463 rows and 15 columns in the dataset.


**3. What are the data types of the variables in this dataset?**

The data types are numeric, character, and date.
...
ggplot(data = data,
      ...
      geom_point(alpha=0.5)+geom_jitter()+ facet_wrap(~ species)+ labs(x="Weight of each animal in grams", y=NULL, subtitle="Hindfoot Length in mm")
...
Got element_text and angle from help sectioin under theme.
```{r}
#| label: boxplot
...
ggplot(data=data, mapping=aes(x=weight, y=species))+ geom_boxplot(outlier.shape = NA)+ geom_jitter(color="turquoise", alpha=0.5)+labs(x="Species Names", y="Weight in grams",title= "Boxplots")
```
...
species_mod <- aov(weight~species, data=data)
...
Based on a small p-value of 2e-16 (large F=19,916), we have sufficient evidence that at least one rodent species has a different population mean weight from another. 
