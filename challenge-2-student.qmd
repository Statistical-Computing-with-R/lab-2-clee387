...
```{r}
#| label: original boxplot

ggplot(data=data, mapping=aes(x=species, y=weight))+ geom_boxplot(outlier.shape = NA)+ geom_jitter(color="turquoise", alpha=0.5)+labs(x="Species Names", y="Weight in grams",title= "Boxplots")+theme(axis.text.x=element_text(angle=45))
...
```{r}
#| label: side-by-side density (ridge) plots

library(ggridges)

ggplot(data=data, mapping=aes(x=weight, y=species))+ geom_density_ridges()
...
```{r}
#| label: Color by Genus
library(ggplot2)

ggplot(data=data, mapping=aes(x=species, y=weight, color=genus))+ geom_boxplot(outlier.shape = NA)+ geom_jitter(color="turquoise", alpha=0.5)+labs(x="Species Names", y="Weight in grams",title= "Boxplots")+theme(axis.text.x=element_text(angle=45))
...
```{r}
#| label: Color Manually

ggplot(data=data, mapping=aes(x=species, y=weight, color=genus, fill=genus)) + geom_boxplot(outlier.shape = NA)+ geom_jitter( alpha=0.5)+labs(x="Species Names", y="Weight in grams", title= "Boxplots")+theme(axis.text.x=element_text(angle=45))+ scale_color_brewer(palette="Set1")
```
...
```{r}
#| label: Adding Annotations

ggplot(data=data, mapping=aes(x=weight, y=species, color=genus))+ 
  geom_boxplot(outlier.shape = NA)+ 
  geom_jitter(alpha=0.5)+
  labs(x="Species Names", y="Weight in grams",title= "Boxplots")+
   annotate("text", y = 13.5, x = 100, label = "Dipodomys") + 
  annotate("text", y = 11.6, x = 120, label = "Chaetodipus") + 
  annotate("text", y = 10, x = 120, label = "Neotoma") + 
  annotate("text", y = 14.3, x = 120, label = "Onychomys") + 
  annotate("text", y = 3.2, x = 120, label = "Perognathus") + 
  annotate("text", y = 8.2, x = 120, label = "Peromyscus") + 
  annotate("text", y = 9, x = 120, label = "Reithrodontomys") + 
  annotate("text", y = 6.5, x = 120, label = "Sigmodon") + 
  theme(legend.position = "none")
